...
/// Returns true if the last two strings in the vector start with `KODEX`.
pub fn match_1(input: Vec<String>) -> bool {
	todo!()
}

/// Returns true if the first and last string in the vector start with `KODEX`.
...
mod tests {
	...
	fn test_match_1_false() {
		let strs = vec!["Hello".to_string(), "World".to_string(), "KODEX".to_string()];
		assert!(!match_1(strs))
	}
